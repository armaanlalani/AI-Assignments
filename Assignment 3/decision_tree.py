...
def dt_entropy(goal, examples):
    ...
    entropy = 0.
    outcomes = np.zeros(len(goal[1])) # instantiates the two arrays needed for computation
    examples_count = (examples.shape)[0]
    for ex in examples:
        outcomes[ex[-1]] += 1 # updates outcomes based on the goal state
    outcomes = np.where(outcomes==0,1,outcomes) # error handling for the zero case
    entropy = -np.sum((outcomes/examples_count)*np.log2(outcomes/examples_count)) # calculates the entropy
...
def dt_cond_entropy(attribute, col_idx, goal, examples):
    ...
    # INSERT YOUR CODE HERE.
    cond_entropy = 0.
    joint_prob = np.zeros((len(attribute[1]),len(goal[1]))) # instantiates the two arrays needed for computation
    att_prob = np.zeros(len(attribute[1]))
    examples_count = (examples.shape)[0]

    for ex in examples:
        att_prob[ex[col_idx]] += 1 # updates the attribute and outcome count
        joint_prob[ex[col_idx], ex[-1]] += 1
    att_prob = att_prob/examples_count # calculates the respective probabilities
    joint_prob = joint_prob/examples_count
    att_prob = np.where(att_prob==0,1,att_prob) # error handling for the zero case
    cond_entropy = 0
    for i in range(joint_prob.shape[0]):
        for j in range(joint_prob.shape[1]):
            if joint_prob[i,j] == 0: # error handling for the zero case
                continue
            cond_entropy -= joint_prob[i,j] * np.log2(joint_prob[i,j]/att_prob[i]) # calculates the entropy
    return cond_entropy
...
def dt_info_gain(attribute, col_idx, goal, examples):
    ...
    # INSERT YOUR CODE HERE.
    info_gain = 0.
    info_gain = dt_entropy(goal,examples) - dt_cond_entropy(attribute,col_idx,goal,examples) # applies the info gain formula
    return info_gain
...
def dt_intrinsic_info(attribute, col_idx, examples):
    ...
    intrinsic_info = 0.
    examples_count = (examples.shape)[0]
    attributes = np.zeros(len(attribute[1])) # instantiates the array needed for computation
    for ex in examples:
        attributes[ex[col_idx]] += 1 # updates the attribut count
    attributes = np.where(attributes==0,1,attributes) # error handling for the zero case
    intrinsic_info = -np.sum((attributes/examples_count)*np.log2(attributes/examples_count)) # calculates the intrinsic info
...
def dt_gain_ratio(attribute, col_idx, goal, examples):
    ...
    if dt_intrinsic_info(attribute,col_idx,examples) != 0:
        gain_ratio = dt_info_gain(attribute,col_idx,goal,examples)/dt_intrinsic_info(attribute,col_idx,examples) # applies the gain ratio formula
    return gain_ratio
...
def learn_decision_tree(parent, attributes, goal, examples, score_fun):
    ...
    if examples.size == 0:
        node = TreeNode(parent,attributes,parent.examples,True,plurality_value(goal,parent.examples))
        return node
    ...
    if np.all(classifications == classifications[0]):
        node = TreeNode(parent,attributes,examples,True,classifications[0])
        return node
    ...
    node = TreeNode(parent,attributes[best_score[1]],examples,False,0)
    attributes_new = list(attributes)
    del attributes_new[best_score[1]]
        # Now, recurse down each branch (operating on a subset of examples below).
        # You should append to node.branches in this recursion
    for i in range(len(attributes[best_score[1]][1])):
        new = examples[examples[:,best_score[1]] == i].copy()
        if new.size != 0:
            new = np.delete(new,best_score[1],1)
        node.branches.append(learn_decision_tree(node,attributes_new,goal,new,score_fun))
    return node
